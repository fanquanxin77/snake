...
class SnakeGame {
  ...
  draw() {
    ...
    if (this.gameOver) {
      ...
      this.ctx.fillText('按空格键重新开始', this.canvas.width/2 - 150, this.canvas.height/2 + 60);
    }
  }
  ...
  handleTouchMove(event) {
    ...
    if (Math.abs(deltaX) > Math.abs(deltaY)) {
        // 水平滑动
        if (deltaX > 50 && this.direction !== 'left') {
            this.nextDirection = 'right';
        } else if (deltaX < -50 && this.direction !== 'right') {
            this.nextDirection = 'left';
        }
    } else {
        // 垂直滑动
        if (deltaY > 50 && this.direction !== 'up') {
            this.nextDirection = 'down';
        } else if (deltaY < -50 && this.direction !== 'down') {
